...
namespace HexLib {
	...
	HexLib::HexLib(QWidget *parent) : QAbstractScrollArea(parent)
	{
		ioDevice = new QBuffer(this);
		init();
		connect(verticalScrollBar(), &QScrollBar::valueChanged, this, &HexLib::adjustContent);
		connect(horizontalScrollBar(), &QScrollBar::valueChanged, this, &HexLib::adjustContent);
	}
	...
	int HexLib::asciiWidth()
	{
		return (mBytesPerLine * 2 + 1) *pxWidth;
	}

	QByteArray HexLib::data(qint64 pos, qint64 count)
	{
		QByteArray buffer;
	...
	void HexLib::setData(const QByteArray &ba)
	{
		buffer.setData(ba);
		setData(buffer);
	}

	bool HexLib::setData(QIODevice &device)
	{
		ioDevice = &device;
	...
	void HexLib::resizeEvent(QResizeEvent *)
	{
		adjustContent();
	}

	void HexLib::paintEvent(QPaintEvent *)
	{
		...
		for (int col = 0; col < mBytesPerLine / 2; col++) {
			painter.fillRect(x - pxWidth / 2, 0, w, height(), viewport()->palette().color(QPalette::Shadow));
			x += 6 * pxWidth;
		}
